...
class Agent(object):
    ...
    def action_train(self):
        ...
        action = torch.clamp(action, -1.0, 1.0) * self.env.v_limit
        ...
        # print(sum(self.rewards) / len(self.rewards), value)
    ...
    def action_test(self):
        ...
        if self.gpu_id >= 0:
            ...
            pi = Variable(pi)
        action = (mu + sigma.sqrt() * eps).data
        action = (torch.clamp(action, -1.0, 1.0) * self.env.v_limit).cpu().numpy()[0]
        # print(action)
